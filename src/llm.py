...
async def enrich_with_llm(
    ...
    for attempt in range(1, settings.llm_max_retries + 1):
        try:
            ...
            if attempt < settings.llm_max_retries:
                # Add validation error to the next prompt attempt
                messages.append({"role": "assistant", "content": content})
                messages.append({
                    "role": "user", 
                    "content": f"Your previous output failed validation: {e}. Please return a valid JSON object matching the schema exactly."
                })
